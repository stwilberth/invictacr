...
@endpush
<x-app-layout :title="$seoTitle" :description="$seoDescription" :ogImage="$ogImage" :ogImageAlt="$displayTitle" ogType="product" :hideWhatsApp="true" :hide-promo-banner="true" :titleSuffix="false" :head="$lcpImageUrl ? '<link rel=&quot;preload&quot; href=&quot;' . $lcpImageUrl . '&quot; as=&quot;image&quot; fetchpriority=&quot;high&quot;>' : ''" >
    @php
                    ...
                    <div class="flex flex-col items-start gap-4 mb-5">
                        ...
                        <div class="{{ (float) ($apartadoMinimo ?? 0) > 0 ? '' : 'col-span-2' }}">
                            ...
                            <span class="text-2xl md:text-[40px] leading-none font-black text-red-600 dark:text-red-500 tracking-tight">₡{{ number_format($priceAfterDiscount, 0) }}</span>
                             <span class="block text-sm font-bold text-gray-400 mt-1">Pago contra entrega de contado*</span>
                            @if(($product->descuento ?? 0) > 0)
